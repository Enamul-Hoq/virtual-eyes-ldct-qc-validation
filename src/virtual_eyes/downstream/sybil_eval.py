...
from virtual_eyes.downstream.pooling import patient_pooling_eval

# ---------------------------
# EDIT THESE PATHS
# ---------------------------
DRIVE_BASE = "/content/drive/MyDrive/MIDL_PAPER"
RESULTS_DIR = os.path.join(DRIVE_BASE, "sybil_eval_results")

PATHS = {
    "PREPROC_CANCER": os.path.join(DRIVE_BASE, "Preprocessed_Cancer_Sybil"),
    "PREPROC_NONCANCER": os.path.join(DRIVE_BASE, "Preprocessed_No_Cancer_Sybil"),
    "RAW_CANCER": os.path.join(DRIVE_BASE, "Raw_Cancer_Sybil"),
    "RAW_NONCANCER": os.path.join(DRIVE_BASE, "Raw_No_Cancer_Sybil"),
}
...
def run_single_condition(prefix: str, cancer_dir: str, noncancer_dir: str) -> dict:
    ...
    model = build_mlp(input_dim=INPUT_DIM, hidden_dims=[64, 32], dropout=0.2, learning_rate=1e-4)
    ...
    pooled_auc = patient_pooling_eval(y_test, y_prob_test, p_test)
...
def main():
    ...
    print("Starting Sybil evaluation...")
    ...
    save_json(
        ...
        os.path.join(RESULTS_DIR, "sybil_final_summary.json"),
    )
    print("Sybil evaluation done.")
